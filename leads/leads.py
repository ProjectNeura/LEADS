...
class Leads(Context[T]):
    ...
    def update(self):
        ...
        if front_wheel_speed := self._require_data("front_wheel_speed"):
            # DTCS
            if (self.is_dtcs_enabled() and
                    self.in_srw_mode() and
                    (rear_wheel_speed := self._require_data("rear_wheel_speed"))):
                if front_wheel_speed < rear_wheel_speed:
                    self._event_listener.on_intervene(InterventionEvent(self,
                                                                        ...
                                                                        rear_wheel_speed))
            if (self.is_dtcs_enabled() and
                    not self.in_srw_mode() and
                    (left_rear_wheel_speed := self._require_data("left_rear_wheel_speed"))):
            ...
            if (self.is_dtcs_enabled() and
                    not self.in_srw_mode() and
                    (right_rear_wheel_speed := self._require_data("right_rear_wheel_speed"))):
